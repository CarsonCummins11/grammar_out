...
def make_follow_set(context) -> List[str]:
    return ["hello", "world", "this", "is", "a", "test", "of", "the", "follow", "set"]
...
def run(args):
    ...
    # read in the grammar file and precompute the first and follow sets
    with open(grammar_file, "r") as file:
        grammar_src = file.read()
        grammar = Grammar.from_string(grammar_src)
        for token in vocab:
            grammar.first(token)
            grammar.follow(token)
    ...
    while pos < steps:
        ...
        if pos < len(prompt_tokens):
            ...
            print(f'{len(state.logits)}')
            possible_tokens = grammar.follow(current_context[-1])
            print(f'{len(possible_tokens)} out of {config.vocab_size} available')
            # only consider logits corresponding to vocab in the follow set
            for i in range(config.vocab_size):
                if vocab[i] not in possible_tokens:
                    state.logits[i] = -1e10 # get rid of this token (hopefully?)
        ...
        current_context.append(token_str)
        print(token_str, end="")
        sys.stdout.flush()
